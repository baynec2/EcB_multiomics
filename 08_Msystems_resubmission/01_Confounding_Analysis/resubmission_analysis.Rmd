...
all = dplyr::inner_join(infected_vs_healthy,death_vs_life,by = "uniprotid") %>% 
  mutate(direction = case_when(death_vs_life_log2fc * infected_v_healthy_log2fc > 0 ~ "same_direction",
         ...
         sig = case_when((infected_v_healthy_adj.p <= 0.05 & death_vs_life_adj.p <= 0.05) ~ "both",
                         ...
                         TRUE ~ "neither")) %>% 
  filter(uniprotid != "Q6UY14")

lm = lm(data = all, infected_v_healthy_log2fc ~ death_vs_life_log2fc)
summary(lm)
...
p1 = all %>% 
  ...
  xlab("log2(Infected/Healthy)")+
  ylab("log2(Mortality/Survival)")
...
lm = lm(data = sig, infected_v_healthy_log2fc ~ death_vs_life_log2fc)
summary(lm)
...
p2 = sig %>% 
  ...
  geom_smooth(method = "lm")+
  geom_abline(slope = 1, linetype = "dashed")+
  ggtitle("Proteins p.adj <= 0.05")+
  xlab("log2(Infected/Healthy)")+
  ylab("log2(Mortality/Survival)")
t2 = gridExtra::tableGrob(as.data.frame(table(sig$direction)))


p3 = ggpubr::ggarrange(p1,p2,common.legend = TRUE,legend = "bottom")


p3

ggsave("proteomics_correlation.pdf",p3, height = 4, width = 8)

ggsave("proteomics_table.pdf",t2)
...
normalized_data = inner_join(gnps_annotations,normalized_data,by = "row_id")

# Performing statistics so we can make a volcano plot
  stat = normalized_data %>%
    dplyr::group_by(row_id,compound_name_cleaned) %>%
...
log2fc_data = normalized_data %>%  
    dplyr::select(metabolomics_id,row_id,compound_name_cleaned,death_during_admission,norm_value) 
...
log2fc = inner_join(dead,live,by = "row_id") %>% 
  mutate(log2fc_dead_live = log2(dead/live))
...
all = inner_join(mort,healthy, by = "row_id") %>% 
    mutate(direction = case_when(log2fc_infected_healthy * log2fc_dead_live > 0 ~ "same_direction",
                                   TRUE ~ "different_direction")) %>% 
...
expected_confounders = c("dibutyl phthalate","phthalic anhydride")

expected_true = c("cholic_acid",
                  "taurocholic acid",
                  "glycocholic acid",
...
labs = all %>% 
  filter(compound_name_cleaned %in% c(expected_true,expected_confounders))

lm = lm(data = all, log2fc_infected_healthy ~ log2fc_dead_live)
summary(lm)


p1 = all %>% 
  ggplot(aes(log2fc_infected_healthy,log2fc_dead_live))+
  geom_point(aes(color = direction))+
  geom_abline(slope = 1, linetype = "dashed")+
  ggrepel::geom_text_repel(data = labs, aes(label = compound_name_cleaned), size = 2)+
  geom_smooth(method = "lm")+
  ggtitle("All Annotated Metabolites") + 
  xlab("log2(Infected/Healthy)")+
  ylab("log2(Mortality/Survival)")
...
lm = lm(data = sig, log2fc_infected_healthy ~ log2fc_dead_live)
summary(lm)
...
p2 = sig %>% 
  ...
  geom_point(aes(color = direction))+
  geom_abline(slope = 1, linetype = "dashed")+
  ggrepel::geom_text_repel(aes(label = compound_name_cleaned), size = 2)+
  geom_smooth(method = "lm")+
  ggtitle("Metabolites p.adj <= 0.05")+
  xlab("log2(Infected/Healthy)")+
  ylab("log2(Mortality/Survival)")
...
ggsave("metabolomics_correlation.pdf",p3, height = 4, width = 8)

ggsave("metabolomics_table.pdf",t2)
