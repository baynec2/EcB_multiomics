...
write_csv(shiny_out,"04_shiny_app/Biomarker_Explorer/www/healthy_prot_EFS_rank.csv")
...
shiny_out = top %>% 
  tibble::rowid_to_column(var = "rank") %>% 
  dplyr::rename(Feature = row_id)
write_csv(shiny_out,"04_shiny_app/Biomarker_Explorer/www/infected_metabolomics_EFS.csv")
```
...
write_csv(shiny_out,"04_shiny_app/Biomarker_Explorer/www/EcB_prot_EFS_rank.csv")
```
...
shiny_out = top %>% 
  tibble::rowid_to_column(var = "rank") %>% 
  dplyr::rename(Feature = row_id)
write_csv(shiny_out,"04_shiny_app/Biomarker_Explorer/www/EcB_metabolomics_EFS.csv")
```
...
f6b = clusterProfiler::cnetplot(enrich, categorySize="pvalue",circular = FALSE,categorySize="pvalue",cex_label_category = 0.8,showCategory = 100,cex_label_gene = 0.5)+
  theme(legend.position = "none")

f6b 

ggsave("02_figures/f6_prediction_of_clinical_outcome/f6b.pdf",f6b,height = 6, width = 6)
...
ggsave("02_figures/f6_prediction_of_clinical_outcome/f6c.pdf",f6c,width = 6, height = 6)
```
...
clean_clinical = inner_join(pl,md_final, by = "tmt_id") %>% 
  ...
  filter(Condition != "Healthy") %>%
  #These get identified by EFS even though it is a terrible biomarker. Not sure why, just going to remove them
  filter(Gene != "IGKC") %>%  filter(Gene != "SOD1") %>%  filter(Gene != "CD5L") %>% filter(Gene != "IGHM") %>% 
   # - messes up EFS for some reason, very annoying. 
  mutate(Gene = gsub("-","_",Gene)) %>% 
...
write_csv(shiny_out,"04_shiny_app/Biomarker_Explorer/www/mortality_prot_EFS_rank.csv")
```
...
topvp = make_violin_plot_metabolomics(data,ncol = 2,topn = topn[1:2],pname = "p.signif")
bottom = make_roc_metabolomics(data,ncol = 2)




f6f = ggpubr::ggarrange(topvp,bottom,nrow = 2)
...
write_csv(shiny_out,"04_shiny_app/Biomarker_Explorer/www/mortality_metabolomics_EFS.csv")
...
s7c = stat_final %>% 
  ...
  geom_hline(yintercept = -log10(0.05),linetype = "dashed", color = "red")+
  coord_flip()+
  facet_wrap(~factor(Gene,topn),nrow = 2) +
  xlab("metadata_field")
...
stat = metadata_assesment(md_assesment_data,
  ...
  #ordering ids 
  ordered = read_csv("01_r_processed_data/top10_live_v_dead_metabolomics_biomarkers.csv") %>% 
  inner_join(gnps_annotations,by = "row_id") %>% 
...
s7d = stat_final %>% 
  ...
  geom_hline(yintercept = -log10(0.05),linetype = "dashed", color = "red")+
  coord_flip()+
  facet_wrap(~factor(compound_name_cleaned,ordered),nrow = 2) +
  xlab("metadata_field")
...
```{r,fig.height = 10, fig.width = 30, eval = FALSE}
